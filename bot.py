...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"» # 𓆩𝗖𝑌𝐵𝐸𝑅𝗗𝑅𝐴𝐺𝑂𝑁𓆪⚡🐲💻 ᴏɴ ᴛᴏᴘ", parse_mode=None, link_preview=None)
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"__🤖 ᴅᴏɴ'ᴛ ᴡᴏʀʀʏ ʙʀᴏ ɪ'ᴍ ᴀʟɪᴠᴇ⚡⚡⚡__\n» `{mp} ms`")
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f" ᴀʟʀɪɢʜᴛ ᴡᴀɪᴛ ғᴏʀ 1 ᴍɪɴ ʙᴏᴛs ᴀʀᴇ ʀᴇsᴛᴀʀᴛɪɴɢ😪")
        try:
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ok = await event.reply(f"» __ɴᴇᴡ ᴄᴏᴍᴍᴀɴᴅᴇʀ sᴘᴏᴛᴛᴇᴅ⚡⚡⚡...__")
        mks = "SUDO_USER"
        target = ""
        ...
        try:
            ...
            await ok.edit("» ɢɴᴅᴜ ᴋɪsᴘᴇ ʀᴀɪᴅ ᴍʀᴜ? ᴛᴀɢ ᴛᴏʜ ᴋʀ ᴜssᴇ !!")
        if len(sudousers) > 0:
            newsudo = f"{sudousers} {target}"
        else:
            newsudo = f"{target}"
        await ok.edit(f"» **ɴᴇᴡ ᴄᴏᴍᴍᴀɴᴅᴇʀ**: `{target}`\n» `ᴡᴀɪᴛ 1ᴍɪɴ ʙᴏᴛs ᴀʀᴇ ʀᴇsᴛᴀʀᴛɪɴɢ ⚡⚡⚡...`")
